...
def run_eval_cam(args, is_egohands, is_egohos_hands, is_egohos_handobject, is_visor_hos, print_log=False):
    ...
    for i, id in enumerate(eval_list):
        ...
        gt = np.asarray(Image.open(gt_file).resize((ori_width,ori_height), resample=Image.NEAREST), dtype=np.int32)##
    ...
    if is_egohands:
        iou = scores(labels, preds, n_class=5)

        metric_path = '/'.join(args.cam_out_dir.split('/')[:-1])
        metric_path = os.path.join(metric_path,str(args.cam_out_dir.split('/')[-1])+'_metric_seed.txt')
        
        with open(os.path.join(metric_path),'w') as val_f:
            val_f.write(str(iou))

    if is_egohos_hands:
        iou = scores(labels, preds, n_class=3)
    ...
    if is_egohos_handobject:
        iou = scores(labels, preds, n_class=6)
    ...
    if is_visor_hos:
        iou = scores(labels, preds, n_class=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--split_file", default='/data1/zhaofeng/Ego_seg/VISOR_HOS_VOC/ImageSets/Segmentation/train.txt', type=str)
    ...
    if 'bg' in args.cam_type or 'png' in args.cam_type:
        ...
        for thres in thres_list:
            ...
            iou = run_eval_cam(args, is_egohands, is_egohos_hands, is_egohos_handobject, is_visor_hos, print_log=False)
            print(thres, iou)
            if iou > max_iou:
                max_iou = iou
                max_thres = thres
        ...
        iou = run_eval_cam(args, is_egohands, is_egohos_hands, is_egohos_handobject, is_visor_hos, print_log=True)
