...
from clip_text import BACKGROUND_CATEGORY_EgoHOS_handobject,  new_class_names_egohos_handobject
import argparse
...
def perform(process_id, dataset_list, args, model, bg_text_features, fg_text_features, cam):
    ...
    for im_idx, im in enumerate(tqdm(databin)):
        ...
        label_list_total = new_class_names_egohos_handobject
        label_id_list_total = [0,1,2,3,4]

        label_root = args.img_root.split('/')[:-1]
        label_root = os.path.join('/'.join(label_root),'SegmentationClassgray_handobject')
        ...
        for image in ms_imgs:
            ...
            for idx, label in enumerate(label_list):
                keys.append(new_class_names_egohos_handobject.index(label))
                ...
                if idx == 0:
                    ...
                    attn_weight = torch.stack(attn_weight, dim=0)[-4:] ## 4 2 ######
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_root', type=str, default='/data/zhaofeng/Ego_seg/EgoHOS/JPEGImages')
    parser.add_argument('--split_file', type=str, default='/data/zhaofeng/Ego_seg/EgoHOS/ImageSets/Segmentation/train.txt')
    ...
    print(BACKGROUND_CATEGORY_EgoHOS_handobject)
    print(new_class_names_egohos_handobject)
    bg_text_features = zeroshot_classifier(BACKGROUND_CATEGORY_EgoHOS_handobject, [args.template], model)
    fg_text_features = zeroshot_classifier(new_class_names_egohos_handobject, [args.template], model)
