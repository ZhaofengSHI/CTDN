...
from clip_text import BACKGROUND_CATEGORY_EgoHOS_hands,  new_class_names_egohos_hands
import argparse
...
def perform(process_id, dataset_list, args, model, bg_text_features, fg_text_features, cam):
    ...
    for im_idx, im in enumerate(tqdm(databin)):
        ...
        label_list_total = new_class_names_egohos_hands
        label_id_list_total = [0,1]

        label_root = args.img_root.split('/')[:-1]
        label_root = os.path.join('/'.join(label_root),'SegmentationClassgray_twohands')
        ...
        for image in ms_imgs:
            ...
            for idx, label in enumerate(label_list):
                keys.append(new_class_names_egohos_hands.index(label))
                ...
                if idx == 0:
                    ...
                    attn_weight = torch.stack(attn_weight, dim=0)[-4:] ## 8
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_root', type=str, default='/data1/zhaofeng/Ego_seg/EgoHOS/JPEGImages')
    parser.add_argument('--split_file', type=str, default='/data1/zhaofeng/Ego_seg/EgoHOS/ImageSets/Segmentation/train.txt')
    ...
    print(BACKGROUND_CATEGORY_EgoHOS_hands)
    print(new_class_names_egohos_hands)
    bg_text_features = zeroshot_classifier(BACKGROUND_CATEGORY_EgoHOS_hands, [args.template], model)
    fg_text_features = zeroshot_classifier(new_class_names_egohos_hands, [args.template], model)
