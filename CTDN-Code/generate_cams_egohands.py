...
from clip_text import BACKGROUND_CATEGORY_Egohands, new_class_names_egohands#, imagenet_templates
import argparse
...
def perform(process_id, dataset_list, args, model, bg_text_features, fg_text_features, cam):
    ...
    for im_idx, im in enumerate(tqdm(databin)):
        ...
        # label_list_total = ['my left hand', 'my right hand', 'left hand of the opposite person', 'right hand of the opposite person']
        label_list_total = new_class_names_egohands
        ...
        label = np.asarray(Image.open(label_path).resize((ori_width,ori_height), resample=Image.NEAREST), dtype=np.int32)#, resample=Image.NEAREST
        ...
        for image in ms_imgs:
            ...
            for idx, label in enumerate(label_list):
                keys.append(new_class_names_egohands.index(label))
                ...
                if idx == 0:
                    ...
                    attn_weight = torch.stack(attn_weight, dim=0)[-2:]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_root', type=str, default='/data/zhaofeng/Ego_seg/EgoHands_new/JPEGImages')
    parser.add_argument('--split_file', type=str, default='/data/zhaofeng/Ego_seg/EgoHands_new/ImageSets/Segmentation/train.txt')
    ...
    print(BACKGROUND_CATEGORY_Egohands)
    print(new_class_names_egohands)
    bg_text_features = zeroshot_classifier(BACKGROUND_CATEGORY_Egohands, [args.template], model)
    fg_text_features = zeroshot_classifier(new_class_names_egohands, [args.template], model)
